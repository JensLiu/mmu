...
#(
  ...
  for (genvar lvl = 0; lvl < LEVELS; lvl++) begin : g_cam_hits
    ...
    always_comb begin
      for (int i = 0; i < TLB_ENTRIES; i++) begin
        hits_per_lvl[lvl][i] = (
            (tlb_entries[i].vpn[VPN_SIZE-1 -: VPN_CMP_BITS] ==
             cache_vpn[VPN_SIZE-1 -: VPN_CMP_BITS])
  ...
  logic
      ...
      store_hit,
      vm_enable,
      tlb_entry_access_is_zero,
      tlb_entry_dirty_is_zero,
      passthrough;
  ...
  for (genvar ppn_l = 0; ppn_l < LEVELS; ppn_l++) begin : g_ppn_lvl
    ...
    if (SUPER_PAGE_BITS == 0) begin : g_kilo
      ...
      assign ppn_per_lvl[ppn_l] = {
        tlb_entries[hit_idx].ppn[PPN_SIZE-1 : SUPER_PAGE_BITS], cache_vpn[SUPER_PAGE_BITS-1 : 0]
      };
    end
  end
