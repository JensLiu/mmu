    ...
    for (genvar p = 0; p < NUM_TLB_PORTS; ++p) begin : g_ppn_assignment
        ...
        for (genvar lvl = 0; lvl < mmu_pkg::LEVELS; ++lvl) begin : g_hit_per_lvl
            assign hit_per_port_per_lvl[p][lvl] =
                read_cam_hit[p] && (tlb_read_level[p] == mmu_pkg::LEVEL_BITS'(lvl));
        end
        ...
        logic [mmu_pkg::PPN_WIDTH-1:0] ppn_translation_mask_per_lvl[mmu_pkg::LEVELS];
        for (genvar lvl = 0; lvl < mmu_pkg::LEVELS; ++lvl) begin : g_ppn_translation_mask
            assign ppn_translation_mask_per_lvl[lvl] =
                {mmu_pkg::PPN_WIDTH{hit_per_port_per_lvl[p][lvl] & vm_enable}};
        end
        always_comb begin : g_ppn_selection
            ppn_translated_per_port[p] = '0;
            for (int l = 0; l < mmu_pkg::LEVELS; l++) begin
